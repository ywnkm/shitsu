...
public interface IEvent<T> {
    ...
    public fun subscribe(context: CoroutineContext = EmptyCoroutineContext, handler: EventHandler<T>): EventJob<T>
    ...
    ): EventJob<T> = subscribe(pair.first, pair.second)

    public operator fun plus(
            handler: EventHandler<T>
    ): EventJob<T> = this + (EmptyCoroutineContext to handler)
